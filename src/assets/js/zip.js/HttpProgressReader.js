            ...
            if (!that.data) {
                ...
                request.addEventListener(
                    ...
                    function () {
                        if (!that.size)
                            that.size =
                                Number(request.getResponseHeader('x-goog-stored-content-length')) ||
                                Number(request.getResponseHeader('x-goog-stored-content-length')) ||
            ...
            request.addEventListener(
                ...
                function () {
                    if (request.status >= 200 && request.status < 300) {
                        that.size = Number(request.getResponseHeader("x-goog-stored-content-length")) || Number(request.getResponseHeader('x-goog-stored-content-length')) || Number(request.getResponseHeader('x-goog-stored-content-length'))
                            ;
                    } else {
                        that.size = 0;
                    }
